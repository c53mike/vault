...
This endpoint creates a named cryptographic key of a specified type. The parameters
...
- `min_enabled_version` `(int: 0)` – Specifies the minimum enabled version of the key. All
  ...
  operations in KMS providers that the key has been distributed to. Setting this value to
...
- `provider` `(string: <required>)` – Specifies the name of a KMS provider that's external to Vault.
  Cannot be changed after creation. The following values are supported:
  - `azurekeyvault` - KMS provider represents a named
...
- `key_collection` `(string: <required>)` – Refers to a location to store keys in the specified
  ...
  depending on the specified provider. The following values are expected for each provider:
  - `azurekeyvault` - The name of an existing Azure Key Vault instance.
...
the KMS provider. The key will still exist in the secrets engine and can be redistributed.
To permanently delete the key from the secrets engine, the
